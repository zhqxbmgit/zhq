...
public class SlideButtonLR extends VirtualControllerElement {
    ...
    private float gestureSlideLeftThresholdPx;
    private float gestureSlideRightThresholdPx;
    ...
    public boolean onElementTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                gestureSlideLeftThresholdPx = preferences.slideButtonLrLeftThresholdDp * density;
                gestureSlideRightThresholdPx = preferences.slideButtonLrRightThresholdDp * density;
                gestureSlideUpThresholdPx = preferences.slideButtonLrUpThresholdDp * density;
                gestureTapHoldMs = preferences.slideButtonLrTapHoldMs;
                gestureLongPressMs = preferences.slideButtonLrLongPressMs;
                downX = event.getX();
                downY = event.getY();
            ...
            case MotionEvent.ACTION_MOVE: {
                if (!slideTriggered) {
                    ...
                    // 左右用各自阈值
                    float lrThreshold = (dx < 0) ? gestureSlideLeftThresholdPx : gestureSlideRightThresholdPx;
                    boolean lrOver = Math.abs(dx) >= lrThreshold;
                    // 上滑(dy<0)只在向上时判定
                    boolean upOver = (dy < 0) && (Math.abs(dy) >= gestureSlideUpThresholdPx);
